...
- **Sacrifice** is the only power that has both no target *and* a non-trivial scoring effect. In `scoring.ts` (last block before `return`): the picker's `delta` is zeroed and every other player's `delta -= picker.number`. The penalty only lands if the picker's own card *would have scored* — it is **nullified (fizzles, no penalty) when the picker is cancelled by a board 0 (`cancelActive`) or their card is tied out** (`pickerTied`), exactly like the things that wipe an ordinary card. Note the tie check is the *picker's own* face being contested: an uninvolved tie among opponents, or multiple zeros that suppress each other (no `cancelActive`), still leave the picker's card live and the sacrifice lands. A `0` sacrifice is a no-op on opponents regardless (it's its own canceller).
