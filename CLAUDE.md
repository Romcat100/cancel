...
- Run a named flow: `npm run verify <flow>` (default `lobby-rounds`). Flows live in the `flows` map at the bottom of the script.
...
- **testid convention**: screen-prefixed kebab-case `<screen>-<element>[-<param>]` — prefixes `home-`, `lobby-`, `game-`, `game-end-`, plus modal prefixes (`power-select-`, `number-select-`, `pool-preview-`, `reveal-`, `round-end-`). Parameterized ids use a **stable** suffix — a card number, a power id, or a player **seat** index — never a uuid or display name. The reusable card/chip components (`NumberCard`, `PowerUpCard`, `PowerUpChip`, `PlayerChip` in `components.tsx`) take an optional `testId` prop, so context-specific ids live at the call site. When adding UI you want to verify, add a stable `data-testid`.
...
- **Power-up pool** (`round.poolFull` / `poolRemaining`) is **public** — face-up in-game.
...
- **Pipeline order** within `scoreTurn`: standard scoring (Plus Two / Minus Two / Drain / Flip face shifts already applied at the eff stage) → Tie Die → Jinx → Double → Make Negative → Free Three → Slide → Equalize → Sacrifice. A new power-up either slots in here, or — like Sabotage / Switch Cards / Random Ray / Swap Hands / Peek — is an engine-level rewrite in `engine.resolveTurn` that bypasses `scoreTurn`.
...
- The non-picker pool uses `<PowerUpChip>` (small chip, symbol only); the picker's uses `<PowerUpCard>` (full card, tap for description). `Pool` switches on `isPicker`. `PowerUpChip` is tappable on mobile to flash the name (desktop uses the `title` tooltip).
- **Power-up modes**: `config.powerUpMode` is `"off" | "random" | "selected"` (plus `config.selectedPowerUps`, the host allow-list). `startRound` deals `[]` for off, a random pool for random, a pool drawn only from `selectedPowerUps` for selected. The pool section, the round-start `PoolPreview`, and the picker label/badge are all gated on `round.poolFull.length > 0`, so the UI reacts to an empty pool rather than branching on the mode. Keep new picker UI behind the same gate.
- **Targeting**: the target-picker block in `Game.tsx` handles all `needsTarget` powers. For Sabotage, after the target is chosen it renders the target's hand as `<NumberCard size="sm">` buttons so the picker forces a card; the submission then carries `powerUp`, `powerUpTarget`, `sabotageNumber`.
...
- **Rules overlay**: `<Rules />` in `components.tsx` auto-renders its power-up section from the live `POWER_UPS` map (new powers appear free), swapped to an "(off)" note when `includePowerUps={false}`. Rendered from a button in `Lobby.tsx` and the `Game.tsx` header.
- **Scope prefix chip**: descriptions render via `<ScopedDescription>`, which parses the leading `(Everyone|Opponents|Just you|Anyone)` tag into a colored chip then the body. Keep the prefix in the `POWER_UPS` string (it's the parser's source of truth) — never print `def.description` raw, or the `(tag)` shows as literal text.
- **Lobby power-up control**: host-only `None / Random / Choose` segmented control calls `api.setConfig` (→ engine `setRoomConfig`, lobby-only). **Choose** seeds `selectedPowerUps` with the full set the first time and opens `PowerSelectModal` (checkbox list of every `POWER_UPS` entry); Peek/Sabotage are disabled in 2-player games (`TWO_PLAYER_EXCLUDED_POWERS`, shared engine/client). Start is blocked when selected mode resolves to an empty pool (`startGame` also throws defensively). Non-hosts see the read-only mode via projected `publicState.config`.
...
3. Add a `POWER_VISUAL` entry in `client/src/components.tsx` (glyph + tailwind colors; long names auto-wrap in the fixed `68×88px` card). If it needs no target, add the id to `SAFE_POOL` in `engine.test.ts` (lifecycle tests draw from it).
