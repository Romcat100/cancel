...
- **testid convention**: screen-prefixed kebab-case `<screen>-<element>[-<param>]` — prefixes `home-`, `lobby-`, `game-`, `game-end-`, plus modal prefixes (`round-power-select-`, `round-power-preview-`, `number-select-`, `reveal-`, `round-end-`; dormant: `power-select-`, `pool-preview-`). Parameterized ids use a **stable** suffix — a card number, a power id, or a player **seat** index — never a uuid or display name. The reusable card/chip components (`NumberCard`, `PowerUpCard`, `PowerUpChip`, `PlayerChip` in `components.tsx`) take an optional `testId` prop, so context-specific ids live at the call site. When adding UI you want to verify, add a stable `data-testid`.
...
Three non-obvious flows (1 and 2 are dormant per-turn paths, reachable only from tests/legacy saves):
...
**Round powers** (`scoreTurn`'s third param, the live system — see `describe("scoreTurn — round powers")`):

- **Pure Tone** (`pure_tone`) is an intentional true no-op — no branch anywhere (the Nothingburger pattern). Don't "fix" it.
- **Harmony** (`harmony`) rewires the tie branches: a face-tied card scores `2` instead of `0`, including matching zeros in the multi-zero-suppress branch. A lone-0 cancel is NOT a tie and is not rescued. **Note-string coupling:** Harmony notes (`"Harmony: tied on N, scored 2"`, `"Harmony: matched zeros scored 2"`) must never start with `"Tied"` or contain `"cancel suppressed"`/`"Cancelled by 0"` — `revealTreatment` keys off those substrings, and with `delta > 0` a Harmony tie deliberately renders as a glowing survivor, not a CANCELLED pair.
- **Static** (`static`) ORs into `negateZeroActive` (same math as per-play Negate Zero, distinct note `"Played 0 (Static, no cancel)"`).
- **Infrared** (`infrared`) ORs into `minusTwoActive` (same math as per-play Minus Two, distinct `"Infrared: X → Y"` notes; a dormant forced Plus Two preempts the −2 for that player — locked by a composition test, don't restructure the eff ternary).
- **Amplify** / **Equalize** are post-passes at the very END of the pipeline (after Sacrifice), with notes `"Amplify: doubled"` / `"Equalize: averaged to N"` distinct from the per-play `"Doubled"` / `"Equalized to avg N"`. Amplify doubles every nonzero delta, negatives included.
...
- **Pipeline order** within `scoreTurn`: standard scoring (Plus Two / Minus Two / Drain / Flip face shifts already applied at the eff stage) → Tie Die → Jinx → Double → Make Negative → Free Three → Slide → Equalize → Sacrifice → **round-power Equalize → round-power Amplify** (the round-power post-passes always run last so they act on the final per-play result).
...
- **Scope prefix chip**: descriptions render via `<ScopedDescription>`, which parses the leading `(Everyone|Opponents|Just you|Anyone)` tag into a colored chip then the body. Keep the prefix in the `POWER_UPS`/`ROUND_POWERS` strings (it's the parser's source of truth) — never print `def.description` raw, or the `(tag)` shows as literal text. Round powers are always `(Everyone)`.
...
1. Add the id to the `RoundPowerId` union and `ROUND_POWERS` map in `shared/types.ts`. The description must start with the `(Everyone)` scope tag (round powers apply to all) and contain no em-dashes.
2. Wire it into `scoreTurn`'s round-power composition points in `scoring.ts`: OR into an existing flag at the top (the Static/Infrared pattern), rewire a branch (the Harmony pattern), or add an end-of-pipeline post-pass (the Amplify/Equalize pattern). Give its notes distinct strings that don't collide with `revealTreatment`'s substrings. Add `scoring.test.ts` cases, including a dormancy-composition case with a forced per-play power.
